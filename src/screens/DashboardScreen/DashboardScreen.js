...
const HomeScreen = () => {
  ...
  const [salesData, setSalesData] = useState(null); // Initially null - to control visibility of containerbox
    ...
    try {
      ...
      if (response.data.success && response.data.sales) {
        setSalesData({
          ...
          pending: response.data.sales.pending ?? 0,
        });
      } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.openDrawer()} style={styles.menuIconImg}>
          <Image source={require('../../assets/menu.png')} style={styles.IconImg} />
        </TouchableOpacity>
      ...
      {/* Show containerbox only if salesData is fetched */}
      {/* {salesData && (
...
const styles = StyleSheet.create({
  ...
  menuIconImg: {
    ...
    position: 'absolute',
    left: wp('5%'),
    top: Platform.OS === 'ios' ? 40 : (StatusBar.currentHeight || 24) + 8,
    zIndex: 4,
  },
  logo: {
    width: wp('15%'),
