...
const UpdateInvoiceScreen = ({ route, navigation }) => {
  const invoiceId = route?.params?.invoiceId || null;
  
  console.log('UpdateInvoiceScreen - Received invoiceId:', invoiceId);
  console.log('UpdateInvoiceScreen - Route params:', route?.params);
  ...
  useEffect(() => {
    if (!invoiceId) {
      Alert.alert(
        'Error',
        'Invoice ID is missing. Cannot load invoice data.',
        [
          {
            text: 'Go Back',
  ...
  const fetchInvoiceData = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success && response.data.invoice) {
        ...
        console.log('Customer Details:', invoiceData.customerDetails);
        console.log('Product Details Count:', invoiceData.productDetails?.length);
        
        // Set invoice details
        setInvoiceDetails({
          billNo: invoiceData.invoiceDetails?.billNo || invoiceData.orderNumber || '',
          date: invoiceData.invoiceDetails?.date 
            ? moment(invoiceData.invoiceDetails.date).format('YYYY-MM-DD') 
            : moment().format('YYYY-MM-DD'),
          remarks: invoiceData.invoiceDetails?.remarks || '',
        });
        console.log('Invoice Details Set:', {
          billNo: invoiceData.invoiceDetails?.billNo || invoiceData.orderNumber,
          date: invoiceData.invoiceDetails?.date,
        });
        ...
        setCustomerDetails({
          customerNameEng: invoiceData.customerDetails?.customerNameEng || '',
          customerNameHin: invoiceData.customerDetails?.customerNameHin || '',
          mobileNumber: invoiceData.customerDetails?.mobileNumber || '',
          address: invoiceData.customerDetails?.address || '',
        });
        console.log('Customer Details Set:', invoiceData.customerDetails);

        // Set products
        if (invoiceData.productDetails && invoiceData.productDetails.length > 0) {
          const formattedProducts = invoiceData.productDetails.map((product, index) => {
            console.log(`Processing Product ${index + 1}:`, product);
            
            // ENHANCED: Auto-detect labour type based on fetched data with priority
            let detectedLabourType = '';
            let labourChargesInPercentage = '';
            let labourChargesInGram = '';
            let labourChargesInRs = '';

            // Check for labourChargesInPercentage first
            if (product.labourChargesInPercentage !== null && product.labourChargesInPercentage !== undefined && product.labourChargesInPercentage > 0) {
              detectedLabourType = 'percentage';
              labourChargesInPercentage = product.labourChargesInPercentage.toString();
            } 
            // Then check for labourChargesInGram
            else if (product.labourChargesInGram !== null && product.labourChargesInGram !== undefined && product.labourChargesInGram > 0) {
              detectedLabourType = 'gram';
              labourChargesInGram = product.labourChargesInGram.toString();
            }
            
            // Set labour charges in rupees from the API response
            if (product.labourChargesInRs !== null && product.labourChargesInRs !== undefined) {
              labourChargesInRs = product.labourChargesInRs.toString();
            }

            console.log(`Product ${index + 1} Labour Type Detection:`, {
              detectedLabourType,
            ...
            return {
              ...
              piece: product.piece ? product.piece.toString() : '',
              grossWeightInGrams: product.grossWeightInGrams ? product.grossWeightInGrams.toString() : '',
              netWeightInGrams: product.netWeightInGrams ? product.netWeightInGrams.toString() : '',
              lessWeightInGrams: product.lessWeightInGrams ? product.lessWeightInGrams.toString() : '',
              ratePerGram: product.ratePerGram ? product.ratePerGram.toString() : '',
              stoneRate: product.stoneRate ? product.stoneRate.toString() : '',
              value: product.value ? product.value.toString() : '',
              labourChargesInPercentage: labourChargesInPercentage,
              labourChargesInGram: labourChargesInGram,
              labourChargesInRs: labourChargesInRs, // FIXED: Changed to labourChargesInRs
              finalAmount: product.finalAmount ? Math.floor(parseFloat(product.finalAmount)).toString() : '',
              additionalAmount: product.additionalAmount ? product.additionalAmount.toString() : '',
              discountAmount: product.discountAmount ? product.discountAmount.toString() : '',
              labourType: detectedLabourType,
            };
          });
        ...
        console.error('Invalid response structure. Expected response.data.invoice');
      ...
      if (error.response) {
        ...
        Alert.alert('Error', error.response.data?.message || 'Failed to load invoice data');
      } else if (error.request) {
        console.error('No response received:', error.request);
        Alert.alert('Error', 'Network error. Please check your connection.');
      } else {
        console.error('Request setup error:', error.message);
        Alert.alert('Error', 'Failed to setup request');
      }
    } finally {
      setLoading(false);
      console.log('=== FETCH INVOICE DATA COMPLETE ===');
    }
  };
  ...
  // MODIFIED: Helper function to format final amount as integer only
  const formatFinalAmountInteger = (value) => {
    if (value == null || value === undefined || value === '') {
      return '';
    }
    ...
    // Return only integer part, removing all decimal values
  ...
  // MODIFIED: Helper function for final amount input to only accept integers
  const ensureFinalAmountInteger = (value) => {
    if (value == null || value === undefined) {
      return '';
    }
    const stringValue = String(value);
  ...
  const handleChange = (index, key, value) => {
    ...
    const decimalFields = ['ratePerGram', 'labourChargesInPercentage', 'labourChargesInGram', 'labourChargesInRs']; // FIXED: Changed to labourChargesInRs
    const integerFields = ['piece', 'stoneRate', 'additionalAmount', 'discountAmount'];
    ...
    // MODIFIED: Special handling for finalAmount to remove decimals
    if (key === 'finalAmount') {
      value = ensureFinalAmountInteger(value);
    ...
    const rate = parseFloat(updatedProducts[index].ratePerGram) || 0; // UPDATED: Changed to parseFloat for decimal support
    ...
    // Only auto-calculate if the field is not being manually edited
    if (key !== 'value') {
    ...
    if (key !== 'labourChargesInRs') {
      if (currentLabourType === 'percentage' && key === 'labourChargesInPercentage') {
        updatedProducts[index].labourChargesInGram = '';
        const labourAmount = (labourPercentage / 100) * parseFloat(updatedProducts[index].value || 0);
        updatedProducts[index].labourChargesInRs = labourAmount.toFixed(2); // FIXED: Changed to labourChargesInRs
      } else if (currentLabourType === 'gram' && key === 'labourChargesInGram') {
        updatedProducts[index].labourChargesInPercentage = '';
        const labourAmount = labourGram * net;
        updatedProducts[index].labourChargesInRs = labourAmount.toFixed(2); // FIXED: Changed to labourChargesInRs
      } else if (!value && (key === 'labourChargesInPercentage' || key === 'labourChargesInGram')) {
        if (key === 'labourChargesInPercentage' && labourGram) {
          updatedProducts[index].labourChargesInRs = (labourGram * net).toFixed(2); // FIXED: Changed to labourChargesInRs
        } else if (key === 'labourChargesInGram' && labourPercentage) {
          const labourAmount = (labourPercentage / 100) * parseFloat(updatedProducts[index].value || 0);
          updatedProducts[index].labourChargesInRs = labourAmount.toFixed(2); // FIXED: Changed to labourChargesInRs
        } else {
          updatedProducts[index].labourChargesInRs = ''; // FIXED: Changed to labourChargesInRs
        }
    ...
    // MODIFIED: Only auto-calculate final amount if not being manually edited, and ensure it's always an integer
    if (key !== 'finalAmount') {
      const val = parseFloat(updatedProducts[index].value) || 0;
      const lab = parseFloat(updatedProducts[index].labourChargesInRs) || 0; // FIXED: Changed to labourChargesInRs

      const finalAmount = Math.max(0, val + stoneRate + lab + addl - disc);
      // Ensure final amount is always an integer
      updatedProducts[index].finalAmount = Math.floor(finalAmount).toString();
    }
  ...
  const toggleDropdownPurity = (item, index = lastIndex) => {
    ...
    if (item === 'Silver') {
      ...
      if (liveGoldRate && liveGoldRate.data) {
        switch (item) {
          case '18k':
            rateValue = (liveGoldRate.data.price18k / 10).toFixed(2); // UPDATED: Keep decimal values
            break;
          case '20k':
            rateValue = (liveGoldRate.data.price20k / 10).toFixed(2); // UPDATED: Keep decimal values
            break;
          case '22k':
            rateValue = (liveGoldRate.data.price22k / 10).toFixed(2); // UPDATED: Keep decimal values
            break;
          case '24k':
            rateValue = (liveGoldRate.data.price24k / 10).toFixed(2); // UPDATED: Keep decimal values
            break;
          default:
            rateValue = '';
        }
  ...
  const toggleDropdownLabourType = (item, index = lastIndex) => {
    ...
    // Clear the opposite field when selecting a labour type
    if (item === 'percentage') {
    ...
    handleChange(index, 'labourChargesInRs', ''); // FIXED: Changed to labourChargesInRs
  };
  ...
  const calculateTotals = () => {
    ...
    products.forEach(product => {
      // MODIFIED: Ensure final amount is treated as integer
      const amount = Math.floor(parseFloat(product.finalAmount)) || 0;
  ...
  const formatInvoiceData = () => {
    ...
    const formattedData = {
      invoiceDetails: {
        ...
        remarks: invoiceDetails.remarks || "Invoice updated from mobile app"
      },
      ...
      productDetails: products.map(product => ({
        ...
        ratePerGram: parseFloat(product.ratePerGram) || 0, // UPDATED: Changed to parseFloat for decimal support
        additionalAmount: parseInt(product.additionalAmount) || 0,
        discountAmount: parseInt(product.discountAmount) || 0,
        labourChargesInRs: parseFloat(product.labourChargesInRs) || 0 // FIXED: Added labourChargesInRs
      }))
    };
  ...
  // Function to scroll to specific input
  ...
  // Handle input focus with proper scrolling
  const handleInputFocus = (inputKey) => {
    scrollToInput(inputKey);
  };

  // Focus on specific product field
  ...
  const navigateToPayment = async () => {
    if (!invoiceId) {
      Alert.alert('Error', 'Invoice ID is missing. Cannot proceed to payment.');
      return;
    }
    ...
    navigation.navigate('update-invoice-bill', {
      invoiceId: invoiceId,
      invoiceDetails,
      customerDetails: processedCustomerDetails,
  ...
  const getRate = (key) => {
    ...
    if (liveGoldRate && liveGoldRate.data && liveGoldRate.data[key]) {
      const rate = parseFloat(liveGoldRate.data[key]).toFixed(2); // UPDATED: Keep decimal values
      return `₹${rate}`;
    }
  ...
  // Add early return if no invoiceId and loading is false
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.errorContainer}>
          <Text style={styles.errorText}>Invoice ID is missing</Text>
            ...
            <View style={styles.sectionFirst}>
              ...
              <View style={styles.inputContainerHalf}>
                ...
                <TextInput
                  ...
                  editable={false} // Date remains non-editable
                />
                ...
                <View style={styles.section} key={index}>
                  ...
                  {!isExpanded && hasData && (
                    <View style={styles.productBox}>
                      ...
                      {/* MODIFIED: Ensure final amount is displayed as integer */}
                      {item.finalAmount ? <Text style={styles.bottomRight}>Amount: ₹{Math.floor(parseFloat(item.finalAmount) || 0)}</Text> : null}
                    </View>
                  )}
                      ...
                      <View style={styles.expandedProductContent}>
                        ...
                        <View style={styles.inputContainer}>
                          ...
                          <TextInput
                            ...
                            editable={false} // Net weight remains auto-calculated
                          />
                        </View>
                        ...
                        <View style={styles.inputContainer}>
                          ...
                          <TextInput
                            ...
                            keyboardType="decimal-pad" // UPDATED: Changed to decimal-pad for decimal input
                        ...
                        <View style={styles.inputContainer}>
                          ...
                          <TextInput
                            ...
                            keyboardType="numeric"
                            editable={true} // Made editable as requested
                          />
                        </View>
                        ...
                        {/* MODIFIED: Final Amount field - only accepts integers, no decimals */}
                        <View style={styles.inputContainer}>
                          <Text style={styles.label}>Final Amount</Text>
                          <TextInput
                            style={styles.input}
                            value={formatFinalAmountInteger(item.finalAmount || '0')}
            ...
            {productForm && lastIndex >= 0 && (
              <View>
                ...
                <View style={styles.inputContainer}>
                  ...
                  <TextInput
                    style={styles.input}
                    value={String(products[lastIndex].labourChargesInRs || '')} // FIXED: Changed to labourChargesInRs
                ...
                {/* MODIFIED: Final Amount field in product form - only accepts integers */}
                <View style={styles.inputContainer}>
                  <Text style={styles.label}>Final Amount</Text>
                  <TextInput
                    style={styles.input}
                    value={formatFinalAmountInteger(products[lastIndex].finalAmount || '0')}
              ...
              <View style={styles.totalSection}>
                ...
                <View style={styles.totalContainer}>
                  ...
                  {products.map((product, index) => {
                    // MODIFIED: Ensure amount is treated as integer in summary
                    const hasValidAmount = product.finalAmount && Math.floor(parseFloat(product.finalAmount)) > 0;
        ...
        <View style={styles.footer}>
          ...
          >
            {isPaymentLoading ? (
              ...
              <Text style={styles.payText}>Update Invoice</Text>
            )}
          </TouchableOpacity>
        </View>
      </SafeAreaView>
