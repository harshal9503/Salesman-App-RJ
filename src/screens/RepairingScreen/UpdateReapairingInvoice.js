...
const UpdateRepairingInvoice = ({ navigation, route }) => {
  // Debug route params
  console.log('UpdateRepairingInvoice - Route params:', route.params);
  
  const { repairInvoiceId } = route.params || {};
  console.log('UpdateRepairingInvoice - repairInvoiceId:', repairInvoiceId);
  ...
  useEffect(() => {
    const fetchInvoiceData = async () => {
      console.log('Starting API call with repairInvoiceId:', repairInvoiceId);
      
      try {
        setLoading(true);
        
        if (!repairInvoiceId) {
          console.error('No repairInvoiceId provided');
        ...
        if (result.success && result.repairInvoice) {
          const invoiceData = result.repairInvoice;
          console.log('Invoice data:', JSON.stringify(invoiceData, null, 2));

          // Auto-fill customer details
          console.log('Setting customer details:', invoiceData.customerDetails);
          setCustomerDetails({
            customerNameEng: invoiceData.customerDetails.customerNameEng || '',
            customerNameHin: invoiceData.customerDetails.customerNameHin || '',
            mobileNumber: invoiceData.customerDetails.mobileNumber?.toString() || '',
            address: invoiceData.customerDetails.address || '',
          });

          // Auto-fill invoice details with formatted date
          console.log('Original date:', invoiceData.invoiceDetails.date);
          const formattedDate = moment(invoiceData.invoiceDetails.date).format('DD-MM-YYYY');
          console.log('Formatted date:', formattedDate);
          setInvoiceDetails({ date: formattedDate });
          setDate(new Date(invoiceData.invoiceDetails.date));

          // Auto-fill salesman contact number
          console.log('Salesman contact:', invoiceData.salesmanContactNumber);
          setSalesmanContactNumber(invoiceData.salesmanContactNumber?.toString() || '');

          // Auto-fill product details
          if (invoiceData.productDetails && invoiceData.productDetails.length > 0) {
            console.log('Product details:', JSON.stringify(invoiceData.productDetails, null, 2));
            
            const formattedProducts = invoiceData.productDetails.map((product, index) => {
              console.log(`Formatting product ${index + 1}:`, product);
              return {
                ...
                finalAmount: product.finalAmount ? product.finalAmount.toString() : '',
              };
            });
            ...
            // Initialize expanded state for all products
            const expandedState = {};
            formattedProducts.forEach((_, index) => {
              expandedState[index] = true;
            });
            setExpandedProducts(expandedState);
            
            // Initialize metal values
            const metalState = {};
            const inputClearState = {};
            formattedProducts.forEach((product, index) => {
              metalState[index] = product.metal || '';
              inputClearState[index] = !product.metal;
            });
            setMetalValues(metalState);
            setInputClearMetal(inputClearState);
          }
    ...
    // Only call API if we have the invoice ID
    if (repairInvoiceId) {
      fetchInvoiceData();
    } else {
      console.error('No repairInvoiceId provided, not making API call');
      setLoading(false);
    }
  }, [repairInvoiceId]);
  ...
  const navigateToPayment = () => {
    ...
    navigation.navigate('update-second-bill', {
      invoiceDetails,
      customerDetails: { ...customerDetails, customerNameEng: customerNameEngFinal },
      productDetails: filteredProducts,
      salesmanContactNumber,
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.infoText}>
          Please update the required details and save the changes
        </Text>
            ...
            >
              ...
              <View style={styles.productHeaderRight}>
                {products.length > 1 && (
                  ...
                  >
                    <Text style={styles.removeProductText}>Discard</Text>
                  </TouchableOpacity>
                )}
                <Image
                  source={require('../../assets/dropdownicon.png')}
...
export default UpdateRepairingInvoice;